...
cascade = cv2.CascadeClassifier("face_emotion_project/haarcascade_frontalface_default.xml")
...
def load_VGG_16_model():
    model = tf.keras.models.load_model('face_emotion_project/emotion_model.keras', compile=False)  # type: ignore
    print(type(model))
    return model
...
def load_yolo_model():
    yolo_model = torch.hub.load(
        'ultralytics/yolov5', 'custom', path='face_emotion_project/best.pt')
